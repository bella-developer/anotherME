...
/**
 * PostForm Component (Modal)
 * Form for creating new posts with circle creation/selection
 * Requirements: 25.1, 25.2, 25.3, 25.4, 25.5, 25.6, 25.7
 * Requirements: 35.2 - Modal fade-in animations (300ms)
 * Requirements: 35.7 - Respect prefers-reduced-motion
 */
const PostForm = ({ isOpen, onClose, circles = [] }) => {
  ...
  const handleCreateCircle = async () => {
    ...
    if (newCircle.description.length > 500) {
      setValidationErrors(prev => ({
        ...prev,
        circleDescription: 'Description must not exceed 500 characters'
      }));
      return;
    }
    ...
    try {
      ...
      // Set the newly created circle as selected
      setFormData(prev => ({ ...prev, circleId: createdCircle.circle.id }));
      setShowCircleCreation(false);
      setNewCircle({ name: '', description: '' });
      
      // Refresh circles list (parent component should handle this)
      // For now, we just close the creation form
    } catch (err) {
      setValidationErrors(prev => ({
        ...prev,
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#2a2a2a]">
          <h2 id="modal-title" className="text-white text-2xl font-bold">Create Post</h2>
          <button
            onClick={handleClose}
            disabled={loading}
        ...
        <form onSubmit={handleSubmit} className="p-6">
          {/* Room Selector */}
          <div className="mb-4">
            <label className="block text-[#a3a3a3] text-sm mb-2">Room</label>
            <div className="grid grid-cols-3 gap-3">
              {rooms.map((room) => (
                <button
                  key={room.value}
                  type="button"
                  onClick={() => setFormData(prev => ({ ...prev, room: room.value }))}
                  disabled={loading}
                ...
                >
                  ...
                  <div className="text-xs mt-1 opacity-70">{room.description}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="mb-4">
            <label className="block text-[#a3a3a3] text-sm mb-2">Circle for Discussion</label>
            ...
            {!showCircleCreation ? (
              <div className="space-y-2">
                ...
                >
                  <option value="">Select existing circle</option>
                  {circles.map((circle) => {
          ...
          <div className="mb-6">
            ...
            <select
              ...
              disabled={loading}
              className={`w-full bg-[#1a1a1a] text-[#e5e5e5] border ${
                validationErrors.category ? 'border-[#ef4444]' : 'border-[#2a2a2a]'
              } rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-[#D97757] focus:ring-2 focus:ring-[#D97757] focus:ring-opacity-20 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed`}
            >
              {categories.map((cat) => (
            ...
            {validationErrors.category && (
              <span className="text-[#ef4444] text-xs mt-1 block">{validationErrors.category}</span>
            )}
          </div>
